...
int main(){
    ...
    fann_type predict[7] = {1.6,3.9,5.8,7.5,7.1,6.1,5.33};
    normalise_temperature(&(predict[0]), 7);
    printf("Predict In 0: %f\n", *(&(predict[0])+0));
    printf("Predict In 1: %f\n", *(&(predict[0])+1));
    printf("Predict In 2: %f\n", *(&(predict[0])+2));
    printf("Predict In 3: %f\n", *(&(predict[0])+3));
    printf("Predict In 4: %f\n", *(&(predict[0])+4));
    printf("Predict In 5: %f\n", *(&(predict[0])+5));
    printf("Predict In 6: %f\n", *(&(predict[0])+6));
    ...
    ann = fann_create_from_file("gas_trained.net");
